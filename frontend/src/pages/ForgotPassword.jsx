...
export const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Password reset link has been dispatched to your email! Link: reset-token:' + email);
    } catch (err) {
    ...
    <div className="w-screen h-screen flex justify-center items-center bg-bg-light dark:bg-bg-dark overflow-hidden relative font-sans text-slate-800 dark:text-slate-100">
      ...
      <div className="w-full max-w-md p-8 bg-white/40 dark:bg-bg-panelDark/30 backdrop-blur-2xl border border-emerald-500/10 dark:border-white/5 rounded-3xl shadow-2xl relative z-10 mx-4 transition-all">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold mt-2 tracking-tight">Forgot Password</h1>
          <p className="text-xs text-slate-400 mt-1.5 select-none">Submit your email address to recover your account</p>
        </div>
        ...
        {success ? (
          <div className="text-center py-4">
            ...
            <h2 className="font-semibold text-sm mb-2">Request Processed</h2>
            <p className="text-xs text-slate-450 dark:text-slate-400 mb-6 leading-relaxed">
              We have dispatched a secure authentication recovery link to your inbox.
            </p>
            <Link
              to="/reset-password"
              className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-semibold text-xs transition-all shadow-md"
            >
              Go to Reset Form
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400" htmlFor="email">Email Address</label>
